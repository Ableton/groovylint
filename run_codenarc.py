...
def _download_file(url: str, output_dir: str) -> str:
    ...
    try:
        ...
        logging.exception("Download of %s failed with code %d", url, http_error.code)
...
def run_codenarc(args: argparse.Namespace, report_file: str = None) -> str:
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        try:
            ...
            logging.exception("Failed executing command: %s", " ".join(codenarc_call))
            logging.exception(
...
if __name__ == "__main__":
    try:
        ...
        logging.exception("Found %s violation(s)", exception.num_violations)
        sys.exit(1)
